...
def run_aws_configure(key_id: str, secret_id: str):
    subprocess.run(f"aws configure set aws_access_key_id {key_id}", stderr=subprocess.PIPE, shell=True, check=True, text=True)
    subprocess.run(f"aws configure set aws_secret_access_key {secret_id}", stderr=subprocess.PIPE, shell=True, check=True, text=True)
